...
- `README.md`、`docs/` 下的开发文档以及新增的 Markdown 项目文档，必须以中文作为主体语言。
- 专业术语、算法名称、编程语言、代码、命令、路径、配置键、数据库表/字段以及函数、类和模块名称，
  可以保留准确的英文表达，不要求生硬翻译。
- 标题、背景说明、设计理由、执行步骤、状态、风险和结论应优先使用清晰、自然的中文。
- 代码块、CLI 示例、SQL、配置片段和 API identifier 应保持可直接执行或可准确检索，不得为了中文化改写其语法。
- 修改架构、目录、CLI、数据库契约或下游接口时，必须同步维护相关 README 和开发文档；历史记录中的旧路径、
  旧命令或已停止方案如果继续保留，必须明确标注其历史状态，避免被误认为当前实现。
