...
- 最后更新：2026-09-18
- 当前阶段：暂停开发
- 暂停原因：该路线的结构分类、路径指标和持久化契约复杂度过高，超出当前项目聚焦的
  “自适应分段 + Pivot 精炼分段”范围。
- 生产状态：`trend_pattern_v3` 与 `trend_pattern_v4` 均不再属于生产流程；相关实现、CLI
  和 Schema 维护代码在 `MA-PUBLIC-CLEANUP v1.0.0` 中移除。
- 数据状态：清理不主动删除数据库中已经存在的 `trend_pattern_daily` 和
  `trend_pattern_v4_daily` 历史数据。
- 文档定位：本文档仅作为历史设计决策和未来研究参考，不代表当前实现或下游接口。
- 恢复条件：如未来重新启动该路线，应建立新的正式方案并重新审核算法范围、数据库契约和
  `investment_dashboard` 接入方式。
