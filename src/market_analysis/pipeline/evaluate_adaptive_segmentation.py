...
def _validation_config() -> dict[str, Any]:
    return dict(settings.validation.get("adaptive_segmentation", {}))
...
def _default_output_dir(target_date: date, run_started_at: datetime | None = None) -> Path:
    ...
    return (
        _PROJECT_ROOT
        / "artifacts"
        / "adaptive_segmentation_validation"
...
def _production_parameters() -> ParameterSet:
    params = settings.indicators.get("adaptive_segmentation", {})
    lookback = int(params.get("lookbacks", [250])[0])
    fixed_max = params.get("max_segments")
    maximum = (
        int(fixed_max)
        if fixed_max is not None
        else recommended_max_segments(lookback, int(params.get("max_segments_cap", 10)))
    )
    return ParameterSet(
...
def _run_parameter_sets(
    ...
    for parameter_set in pending:
        ...
        log.info(
            "adaptive_segmentation_validation.parameter_set.done",
            parameter_key=parameter_set.key,
...
def _parameter_field_guide(
    ...
    entries = [
        (
            "production",
            "是否为 indicators.adaptive_segmentation 当前生产参数。报告始终保留生产参数用于"
            "对照；它不影响排名计算。",
        ),
        (
            "parameter_key",
...
<title>Adaptive segmentation validation</title>
<style>
...
<body>
<h1>Adaptive segmentation validation</h1>
...
def run_adaptive_segmentation_validation(
    ...
    lookbacks = [
        int(value)
        for value in settings.indicators.get("adaptive_segmentation", {}).get(
            "lookbacks", [250]
        )
    ]
    ...
    if missing:
        log.warning("adaptive_segmentation_validation.symbols.missing", symbols=missing)
    if not frames:
    ...
    log.info(
        "adaptive_segmentation_validation.done",
