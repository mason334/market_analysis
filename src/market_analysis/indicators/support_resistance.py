...
def support_resistance(
    ...
    """
    Compute the support/resistance daily snapshot.

    Returns a dict matching support_resistance_daily columns, or None when the
    ...
    sr_status = _compute_sr_status(current_close, df, levels, atr, warn_mult, watch_mult)
...
def _detect_breakout(
    ...
    """
    Scan the last `window` candles for a close crossing any SR zone boundary.
    """
...
def compute_sr_levels(df: pd.DataFrame, params: dict[str, Any]) -> list[dict[str, Any]]:
    """
    Compute SR level list from OHLCV.
    ...
    if extreme_order >= 2 and len(df) >= extreme_order * 2 + 1:
        ...
        for idx, is_high in [(i, True) for i in ex_high_idx] + [
            (i, False) for i in ex_low_idx
        ]:
...
def compute_raw_swings(
    ...
    """Return all raw swing points within lookback_window bars."""
